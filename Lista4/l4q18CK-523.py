...
class Enquete:
    ...
    def exibir_resultado(self):
        ...
        for jogador, votos in enumerate(self.votos):
            if votos > 0:
                percentual = self.calcular_percentual(votos, total_votos)
                print(f"Jogador {jogador}: {votos} votos ({percentual:.2f}%)")

        jogador_melhor = self.votos.index(max(self.votos))
        votos_melhor = self.votos[jogador_melhor]
        percentual_melhor = self.calcular_percentual(votos_melhor, total_votos)

        print(f"\nO melhor jogador foi o número {jogador_melhor}, com {votos_melhor} votos, correspondendo a {percentual_melhor:.1f}% do total de votos.")
